...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    if hass.data.get(DATA_VOICE) is None:
        hass.data[DATA_VOICE] = Conversation(hass)
...
async def _async_converse(
    ...
    # 插件
    conversation = hass.data[DATA_VOICE]
    text = conversation.trim_char(text)
    ...
    if result is None:
        if intent_response is None:
            # 插件意图
            intent_response = await conversation.async_process(text)
            speech = intent_response.speech.get('plain')
            if speech is not None:
                conversation.update(text, speech.get('speech'))
